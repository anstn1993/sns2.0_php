...
//사용자 페이지의 주인 account
$hostAccount = $jsonData['hostAccount'];
//사용자 페이지에 접근한 사용자 account
$myAccount = $jsonData['myAccount'];
//검색 여부
$isSearched = $jsonData['isSearched'];
//검색어
$searchText = $jsonData['searchText'];
//현재 로드된 가장 마지막 리스트 id
...
//해당 index의 데이터가 테이블 상에서 몇번째에 해당하는지 알아야 한다.
  ...
  $sql = "
    SELECT COUNT(*) FROM follow WHERE id>='{$lastId}' AND followed_account='{$hostAccount}'
  ";
...
if($isSearched === true){
  $sql="
  SELECT*FROM follow
  WHERE followed_account='{$hostAccount}'
  ORDER BY following_account ASC
...
//검색어가 존재하지 않는 경우
  ...
  $json=json_encode(array("requestType"=>$requestType, "followerlist"=>$data), JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);
  echo $json;
